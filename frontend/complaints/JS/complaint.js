...
/* ── NOTIFICATION BELL + PANEL ─────────────────────────────── */
  ...
  if (notifBtn && notifIcon && notifPanel) {
    ...
    function updateUnreadUI() {
      ...
      document.querySelectorAll('.stat-card').forEach(function(card) {
        ...
        if (label && value && label.textContent.trim().toLowerCase() === 'notification') {
          value.textContent = unreadCount;
        }
      });
    }
  ...
  const anonBack = document.getElementById("anonBack");
  const anonContinue = document.getElementById("anonContinue");

  if (anonymousBtn && anonModal) {
    anonymousBtn.addEventListener("click", (e) => {
      e.preventDefault();
      e.stopPropagation();
      anonModal.classList.add("show");
    });
  }

  if (anonBack && anonModal) {
    anonBack.addEventListener("click", () => {
      anonModal.classList.remove("show");
    });
  }
  ...
  if (anonModal) {
    anonModal.addEventListener("click", (e) => {
      if (e.target === anonModal) {
        anonModal.classList.remove("show");
      }
    });
  }
