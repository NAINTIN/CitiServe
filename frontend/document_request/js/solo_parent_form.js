  ...
  { label: "Solo Parent Cetificate",  href: "solo_parent.php" },
  ...
  const uploadFields = [
    ...
    { inputId: 'psaFileInput',     defaultId: 'psaDefault',     previewId: 'psaPreview',     filenameId: 'psaFilename',     boxId: 'psaUpload',     errorId: 'psaError' },
  ];
  ...
  function selectOption(dropdownId, value) {
    ...
    const errorMap = { reasonDropdown: 'reasonError' };
    ...
    if (dropdownId === 'reasonDropdown') {
      const othersInput = document.getElementById('reasonOther');
      const othersError = document.getElementById('reasonOtherError');
      if (value === 'Others') {
  ...
  function validateForm() {
    ...
    const textFields = [
      ...
      { id: 'numChildren',     errorId: 'numChildrenError' },
      { id: 'childrenNames',   errorId: 'childrenNamesError' },
    ];
    ...
    if (!reasonDropdown.dataset.value) {
      reasonError.style.display = 'flex';
      reasonDropdown.querySelector('.custom-select-selected').classList.add('select-error');
      valid = false;
    } else {
      reasonError.style.display = 'none';
      reasonDropdown.querySelector('.custom-select-selected').classList.remove('select-error');
    }

    // Others input if "Others" selected
    if (reasonDropdown.dataset.value === 'Others') {
      const othersInput = document.getElementById('reasonOther');
      const othersError = document.getElementById('reasonOtherError');
    ...
    if (valid) {
    window.location.href = 'docu_business_payment.php?doc=solo_parent';
  }
    return valid;
  }
