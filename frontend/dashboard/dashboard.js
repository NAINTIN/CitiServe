document.addEventListener('DOMContentLoaded', function () {
  ...
  function renderCalendar(year, month) {
    ...
    function addDay(num, extraClass) {
      var el = document.createElement('div');
      el.className = extraClass ? 'cal-day ' + extraClass : 'cal-day';
      el.textContent = num;
      grid.appendChild(el);
    }

    for (var i = firstDay - 1; i >= 0; i--) {
      addDay(daysInPrev - i, 'other-month');
    }

    for (var d = 1; d <= daysInMon; d++) {
      addDay(d, isNow && d === now.getDate() ? 'today' : '');
    }

    var filled   = firstDay + daysInMon;
    var trailing = (filled % 7 === 0) ? 0 : 7 - (filled % 7);

    for (var t = 1; t <= trailing; t++) {
      addDay(t, 'other-month');
    }
  }
